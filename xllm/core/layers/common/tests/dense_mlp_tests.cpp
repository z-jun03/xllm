...
TEST_F(DenseMLPTest, ColumnParallelLinearForwardTpViewSlicesRows) {
  QuantArgs no_quant;
  auto tp_group = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  ColumnParallelLinear linear(2,
                              4,
                              /*bias=*/false,
                              /*gather_output=*/false,
                              no_quant,
                              tp_group.get(),
                              options_,
                              LinearExtraArgs("none", false, true));
  ...
  torch::Tensor input = torch::tensor({{1.0f, 1.0f}}, options_);
  torch::Tensor output = linear->forward(input, /*use_full_w=*/false);

  auto expected = torch::tensor({{11.0f, 15.0f}}, options_);
  verify_tensor_close(output.to(torch::kFloat32), expected.to(torch::kFloat32));
}

TEST_F(DenseMLPTest, ColumnParallelLinearForwardFullWReturnsFullOutput) {
  QuantArgs no_quant;
  auto tp_group = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  ColumnParallelLinear linear(2,
  ...
  torch::Tensor input = torch::tensor({{1.0f, 1.0f}}, options_);
  torch::Tensor output = linear->forward(input, /*use_full_w=*/true);

  auto expected = torch::tensor({{3.0f, 7.0f, 11.0f, 15.0f}}, options_);
  verify_tensor_close(output.to(torch::kFloat32), expected.to(torch::kFloat32));
}

TEST_F(DenseMLPTest, RowParallelLinearForwardTpViewSlicesColumns) {
  QuantArgs no_quant;
  auto tp_group = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  RowParallelLinear linear(4,
                           2,
                           /*bias=*/false,
  ...
  torch::Tensor local_input = torch::tensor({{10.0f, 20.0f}}, options_);
  torch::Tensor output = linear->forward(local_input, /*use_full_w=*/false);

  auto expected = torch::tensor({{110.0f, 230.0f}}, options_);
  verify_tensor_close(output.to(torch::kFloat32), expected.to(torch::kFloat32));
}

TEST_F(DenseMLPTest, RowParallelLinearForwardFullWAcceptsFullWidthInput) {
  QuantArgs no_quant;
  auto tp_group = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  RowParallelLinear linear(4,
  ...
  torch::Tensor full_input =
      torch::tensor({{1.0f, 2.0f, 10.0f, 20.0f}}, options_);
  torch::Tensor output = linear->forward(full_input, /*use_full_w=*/true);

  auto expected = torch::tensor({{115.0f, 247.0f}}, options_);
  verify_tensor_close(output.to(torch::kFloat32), expected.to(torch::kFloat32));
}

TEST_F(DenseMLPTest, RowParallelLinearForwardTpViewAcceptsFullWidthInput) {
  QuantArgs no_quant;
  auto tp_group = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  RowParallelLinear linear(4,
  ...
  torch::Tensor full_input =
      torch::tensor({{1.0f, 2.0f, 10.0f, 20.0f}}, options_);
  torch::Tensor output = linear->forward(full_input, /*use_full_w=*/false);

  auto expected = torch::tensor({{110.0f, 230.0f}}, options_);
  verify_tensor_close(output.to(torch::kFloat32), expected.to(torch::kFloat32));
}

TEST_F(DenseMLPTest, RowParallelLinearForwardTpViewAppliesBiasOnlyOnRankZero) {
  QuantArgs no_quant;
  auto tp_group_rank0 = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 0, 2);
  auto tp_group_rank1 = std::make_unique<xllm::layer::test::MockProcessGroup>(
      options_.device(), 1, 2);
  RowParallelLinear linear_rank0(4,
                                 3,
                                 /*bias=*/true,
                                 /*input_is_parallelized=*/true,
                                 /*enable_result_reduction=*/false,
                                 ...
                                 options_,
                                 LinearExtraArgs("none", false, true));
  RowParallelLinear linear_rank1(4,
                                 3,
                                 /*bias=*/true,
                                 /*input_is_parallelized=*/true,
                                 /*enable_result_reduction=*/false,
  ...
  StateDict state_dict({{"weight", torch::zeros({3, 4}, options_)},
                        {"bias", torch::tensor({1.0f, 2.0f, 3.0f}, options_)}});
  linear_rank0->load_state_dict(state_dict);
  linear_rank1->load_state_dict(state_dict);

  torch::Tensor input = torch::zeros({2, 4}, options_);
  torch::Tensor rank0 = linear_rank0->forward(input, /*use_full_w=*/false);
  torch::Tensor rank1 = linear_rank1->forward(input, /*use_full_w=*/false);

  auto expected_rank0 =
      torch::tensor({{1.0f, 2.0f, 3.0f}, {1.0f, 2.0f, 3.0f}}, options_);
  auto expected_rank1 = torch::zeros({2, 3}, options_);
  verify_tensor_close(rank0.to(torch::kFloat32),
                      expected_rank0.to(torch::kFloat32));
  verify_tensor_close(rank1.to(torch::kFloat32),
                      expected_rank1.to(torch::kFloat32));
}
