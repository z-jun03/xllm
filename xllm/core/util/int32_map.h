...
// ref to:
// https://
// github.com/snowflakedb/ArcticInference/blob/main/csrc/suffix_decoding/int32_map.h
#pragma once
...
class Int32Map {
 public:
  ...
  T& emplace(int32_t key, Args&&... args) {
    ...
    if (slots_[idx].key == KEY_TOMBSTONE) {
      --tombstones_;
      slots_[idx].key = key;
      ::new (static_cast<void*>(&slots_[idx].storage))
  ...
  T& operator[](int32_t key) { return emplace(key); }
 ...
 private:
  ...
  // Either finds existing (true, idx_out set) or returns best insert slot
  // (false). On "not found", idx_out is: first tombstone if any; else first
  // empty.
