...
//     LOG(INFO) << "create_npu_process_groups success.";
//   }
...
//     // CHECK(parallel_args.process_group_ != nullptr) << "Process group is
//     not initialized.";
//     // CHECK(pg!= nullptr) << "Process group 1 is not initialized.";
//   }
//   // CHECK(pg_ != nullptr) << "Process group is not initialized.";
//   // CHECK(parallel_args.process_groups_ != nullptr) << "Process group is not
//   initialized.";
...
DiTEngine::DiTEngine(const runtime::Options& options) : options_(options) {
  ...
  for (const auto device : devices) {
    ...
    if (ret != 0) {
      ...
      LOG(INFO) << "ACL set device id:" << currentDevId << " success.";
    }
#endif
  }
  // SetDefaultAllowInternalFormatDisable();
