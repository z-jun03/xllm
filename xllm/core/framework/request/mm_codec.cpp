...
bool MMCodec::image_decode(const std::string& raw_data, torch::Tensor& t) {
...
bool MMCodec::image_encode(const torch::Tensor& t, std::string& raw_data) {
  if (!valid(t)) {
    return false;
  }
...
bool MMCodec::valid(const torch::Tensor& t) {
