...
pub unsafe extern "C" fn safetensors_names(
    ...
    let c_names = names
        ...
        .map(|name| {
            ...
            let s = CString::from_vec_unchecked(name.clone().to_string().into_bytes());
            let ptr = s.as_ptr();
...
/// Return the number of bits(1byte=8bits) required to represent a single element from the specified dtype
///
/// # Arguments
///
/// * `dtype`: The data type we want to know the number of bits required
///
/// returns: usize Number of bits for this specific `dtype`
///
/// NOTE: F4, F6_E3M2 and F6_E2M3 size is smaller than a full byte of width.
///
#[no_mangle]
