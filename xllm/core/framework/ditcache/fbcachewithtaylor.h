...
class FBCacheWithTaylorSeer : public CacheBase {
 private:
 ...
 public:
  void init(CacheConfig cfg) override {
    residual_diff_threshold = cfg.fbcachewithtaylor.residual_diff_threshold;
    num_inference_steps = cfg.fbcachewithtaylor.num_inference_steps;
    warmup_steps = cfg.fbcachewithtaylor.warmup_steps;

    CacheConfig taylorseerconfig;
    taylorseerconfig.taylorseer.n_derivatives =
        cfg.fbcachewithtaylor.n_derivatives;
    taylorseerconfig.taylorseer.num_inference_steps =
        cfg.fbcachewithtaylor.num_inference_steps;
    taylorseer = std::make_unique<TaylorSeer>();
    taylorseer->init(taylorseerconfig);
  }

  void set_hidden_states_residual(const torch::Tensor& hidden_states_residual) {
    taylorseer->update(hidden_states_residual);
  }

  void set_encoder_hidden_states_residual(
      const torch::Tensor& encoder_hidden_states_residual) {
    return;
  }

  torch::Tensor get_hidden_states_residual() {
    return taylorseer->approximate_value();
  }

  torch::Tensor get_encoder_hidden_states_residual() {
    return get_tensor_or_empty(buffers, "encoder_hidden_states_residual");
  }

  void set_first_hidden_states_residual(
      const torch::Tensor& hidden_states_residual) {
    buffers["first_hidden_states_residual"] = hidden_states_residual;
  }

  torch::Tensor get_first_hidden_states_residual() {
    return get_tensor_or_empty(buffers, "first_hidden_states_residual");
  }

  std::pair<torch::Tensor, torch::Tensor> apply_prev_hidden_states_residual(
      const torch::Tensor& hidden_states,
      const torch::Tensor& encoder_hidden_states) {
    auto hidden_states_residual = get_hidden_states_residual();
  ...
  bool get_can_use_cache(const torch::Tensor& first_hidden_states_residual) {
    if (is_in_warmup()) {
      return false;
    }
    ...
    auto prev_first_hidden_states_residual = get_first_hidden_states_residual();
    if (!prev_first_hidden_states_residual.defined()) {
      return false;
    }
    bool can_use_cache =
        are_two_tensors_similar(prev_first_hidden_states_residual,
                                first_hidden_states_residual,
                                residual_diff_threshold);
    return can_use_cache;
  }

  CacheBlockOut on_after_block(CacheBlockIn blockin) override {
    ...
    if (blockin.block_id == 0 && encoder) {
      ...
      if (first_hidden_states_residual.defined()) {
        can_use_cache = get_can_use_cache(first_hidden_states_residual);
      }
  ...
  bool on_before_block(CacheBlockIn /*blockin*/) override { return use_cache; }
