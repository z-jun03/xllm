...
class FBCache : public CacheBase {
 private:
  int skip_interval_steps_ = 2;
 ...
 public:
  ...
  void set_encoder_hidden_states_residual(
    ...
    buffers["encoder_hidden_states_residual"] = encoder_hidden_states_residual;
  }

  torch::Tensor get_hidden_states_residual() {
    return get_tensor_or_empty(buffers, "hidden_states_residual");
  }
  ...
  bool on_before_step(CacheStepIn stepin) override {
    ...
    use_cache = false;
    if (executed_steps == 1) buffers.clear();
    return false;
  }
