...
def build_wan_blocked_norm_silu_kernel(
    *,
    ...
    if dtype != DEFAULT_DTYPE:
        raise ValueError(
            f"wan_blocked_norm_silu only supports {DEFAULT_DTYPE}, got {dtype}"
        )
    ...
    if plane_elements <= 0 or plane_elements % SPATIAL_TILE != 0:
        raise ValueError(
            f"plane_elements({plane_elements}) must be divisible by {SPATIAL_TILE}"
        )
    ...
    def wan_blocked_norm_silu(
        blocked_source: T.Tensor(
            (channels * temporal * plane_elements,), tensor_dtype
        ),
        norm: T.Tensor((temporal * plane_elements,), acc_dtype),
        gamma: T.Tensor((channels,), tensor_dtype),
        blocked_output: T.Tensor(
            (channels * temporal * plane_elements,), tensor_dtype
        ),
    ):
            ...
            with T.Scope("V"):
                ...
                for local_task in T.serial(task_count):
                    ...
                    for channel_block in T.serial(channel_blocks):
                        input_offset = (
                            (
                                temporal_index * channel_blocks + channel_block
                            )
                            * plane_elements
                            + spatial_offset
                        ) * 16
                        T.copy(
                            blocked_source[
                                input_offset : input_offset + tile_elements
                            ],
                            input_half_ub,
                        )
                        T.tile.cast(
                        ...
                        T.copy(
                            gamma[
                                channel_block * 16 : (channel_block + 1) * 16
                            ],
                            gamma_half_ub,
                        )
...
def build_wan_blocked_norm_silu_causal_input_kernel(
    *,
    ...
    if dtype != DEFAULT_DTYPE:
        raise ValueError(
            "wan_blocked_norm_silu_causal_input only supports "
            f"{DEFAULT_DTYPE}, got {dtype}"
        )
    if spatial_tile not in CAUSAL_SPATIAL_TILES:
        raise ValueError(
            f"spatial_tile({spatial_tile}) must be one of {CAUSAL_SPATIAL_TILES}"
        )
    if vec_core_num <= 0 or vec_core_num % VEC_NUM != 0:
        raise ValueError(
            f"vec_core_num({vec_core_num}) must be positive and divisible by {VEC_NUM}"
    ...
    def wan_blocked_norm_silu_causal_input(
        ...
        with T.Kernel(block_num, is_npu=True) as (block_index, vector_index):
            ...
            spatial_tiles = (
                spatial_elements + spatial_tile - 1
            ) // spatial_tile
            prefix_tasks = channels * 2
            prefix_tasks_per_worker = (
                prefix_tasks + vec_core_num - 1
            ) // vec_core_num
            ...
            with T.Scope("V"):
                ...
                gamma_vector_fp32_ub = T.alloc_ub(
                    (spatial_tile,), acc_dtype
                )
                ...
                for local_task in T.serial(current_count):
                    ...
                    T.copy(
                        gamma[channel_index : channel_index + 1], gamma_half_ub
                    )
                    ...
                    for tile_index in T.serial(spatial_tiles):
                        ...
                        norm_offset = (
                            temporal_index * plane_elements + spatial_offset
                        )
                        source_offset = (
                            (channel_index * temporal + temporal_index)
                            * plane_elements
                            + spatial_offset
                        )
                        output_offset = (
                            (
                                channel_index * (temporal + 2)
                                + temporal_index
                                + 2
                            )
                            * plane_elements
                        ...
                        T.copy(
                            norm[
                                norm_offset : norm_offset + spatial_tile
                            ],
                            norm_fp32_ub,
                        )
                        T.tile.max(norm_fp32_ub, norm_fp32_ub, NORM_EPS)
                        ...
                        T.tile.div(
                            input_fp32_ub, input_fp32_ub, norm_fp32_ub
                        )
                        T.tile.cast(
                            output_half_ub,
                            input_fp32_ub,
                        ...
                        with T.If(temporal_index >= temporal - 2), T.Then():
                            cache_offset = (
                                (
                                    channel_index * next_cache_temporal
                                    + temporal_index
                                    - temporal
                                    + next_cache_temporal
                                )
