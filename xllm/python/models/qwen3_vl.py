...
class Qwen3VLVisionAttention(nn.Module):
    ...
    def __init__(
        ...
        self.pad_to_max = self.head_dim > _MIN_FUSED_HEAD_DIM and self.head_dim < _MAX_FUSED_HEAD_DIM
        if dtype is not None and device is not None:
            self.to(dtype=dtype, device=device)
...
class Qwen3VLVisionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Qwen3VLVisionAttention(hidden_size, num_heads, dtype=dtype, device=device)
        self.norm2 = nn.LayerNorm(hidden_size, eps=1e-6)
        self.mlp = Qwen3VLVisionMLP(hidden_size, intermediate_size, hidden_act, dtype=dtype, device=device)
    ...
    def forward(
        ...
        x = x + self.attn(self.norm1(x), actual_seq, rotary_cos_full, rotary_sin_full)
...
class Qwen3VLVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.rotary_pos_emb = Qwen3VLVisionRotaryEmbedding(head_dim, max_position=8192, dtype=dtype, device=device)
    ...
    @staticmethod
    def _rot_pos_ids(h: int, w: int, spatial_merge_size: int, device: torch.device) -> torch.Tensor:
    ...
    @staticmethod
    def _compute_cu_seqlens(grid_thw: torch.Tensor, device: torch.device) -> torch.Tensor:
        """Cumulative sequence lengths for per-frame varlen attention."""
        cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0]).cumsum(
            dim=0, dtype=torch.int32
        )
        return torch.cat([torch.zeros(1, dtype=torch.int32, device=device), cu_seqlens])
    ...
    def forward(
        ...
        hidden_states = self.patch_embed(hidden_states).view(L, self.hidden_size)
        ...
        head_dim = self.hidden_size // self.num_heads
        rotary_cos_full = torch.cat((rotary_cos, rotary_cos), dim=-1).reshape(1, -1, 1, head_dim)
        rotary_sin_full = torch.cat((rotary_sin, rotary_sin), dim=-1).reshape(1, -1, 1, head_dim)
        ...
        cu_seqlens = self._compute_cu_seqlens(grid_thw.to(device=self.device), self.device)
        actual_seq: list[int] = cu_seqlens[1:].cpu().tolist()

        # Transformer blocks with deepstack feature collection
        deepstack_features: list[torch.Tensor] = []
        for layer_num, blk in enumerate(self.blocks):
            hidden_states = blk(hidden_states, actual_seq, rotary_cos_full, rotary_sin_full)
...
def _scatter_multimodal(
    ...
    deepstack_acc: torch.Tensor | None,
    embeds: torch.Tensor,
    mask: torch.Tensor,
    visual_dim: int,
    num_level: int,
    ...
    """
    if num_level > 0:
        main, multiscale = torch.split(embeds, [visual_dim, visual_dim * num_level], dim=-1)
    else:
        main = embeds
        multiscale = None
...
class Qwen3VLForConditionalGeneration(PyModelBase):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.vision_start_token_id = int(config.get("vision_start_token_id", 151652))
        self.vision_end_token_id = int(config.get("vision_end_token_id", 151653))
    ...
    def get_input_embeddings(
        ...
        deepstack_acc: torch.Tensor | None = None
        if num_level > 0:
        ...
        for embeds, token_id in modalities:
            ...
            if not mask.any():
                continue
            _scatter_multimodal(inputs_embeds, deepstack_acc, embeds, mask, visual_dim, num_level)
        ...
        if deepstack_acc is not None:
            ...
            self.model.deepstack_input_embeds = [deepstack_acc[:, i, :].contiguous() for i in range(num_level)]
        else:
            self.model.deepstack_input_embeds = None
