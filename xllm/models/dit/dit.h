...
#include "core/framework/ditcache/cache_agent.h"
...
class FluxTransformer2DModelImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(const torch::Tensor& hidden_states_input,
    ...
    bool stepcache = CacheAgent::getinstance().on_before_step(stepin_before);
    ...
    if (!stepcache) {
      for (int64_t i = 0; i < transformer_blocks_->size(); ++i) {
        ...
        bool blockcache =
            CacheAgent::getinstance().on_before_block(blockin_before);
        ...
        CacheBlockIn blockin_before1(i, block_in_map);
        CacheBlockOut blockout_after =
            CacheAgent::getinstance().on_after_block(blockin_before1);
      ...
      for (int64_t i = 0; i < single_transformer_blocks_->size(); ++i) {
        CacheBlockIn blockin0(i);
        bool blockcache = CacheAgent::getinstance().on_before_block(blockin0);
        ...
        // single transformer block after
        TensorMap single_block_map = {
            {"hidden_states", hidden_states},
            {"original_hidden_states", original_hidden_states}};
        CacheBlockIn blockin1(i, single_block_map);
        CacheBlockOut blockout1 =
            CacheAgent::getinstance().on_after_block(blockin1);

        if (blockcache) {
          hidden_states = blockout1.tensors.at("hidden_states");
          if (i == 1) LOG(INFO) << step_idx << "cache hit";
        }
      }
    ...
    TensorMap step_after_map = {
        {"hidden_states", hidden_states},
        {"original_hidden_states", original_hidden_states}};
    CacheStepIn stepin1(step_idx, step_after_map);
    CacheStepOut stepout = CacheAgent::getinstance().on_after_step(stepin1);
    hidden_states = stepout.tensors.at("hidden_states");
