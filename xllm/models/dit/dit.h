...
class FluxTransformer2DModelImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(const torch::Tensor& hidden_states_input,
    ...
    bool stepcache = DiTCacheAgent::getinstance().on_before_step(stepin_before);
    ...
    if (!stepcache) {
      for (int64_t i = 0; i < transformer_blocks_->size(); ++i) {
        ...
        CacheBlockOut blockout_after =
            DiTCacheAgent::getinstance().on_after_block(blockin_before1);
      ...
      for (int64_t i = 0; i < single_transformer_blocks_->size(); ++i) {
        ...
        bool blockcache =
            DiTCacheAgent::getinstance().on_before_block(blockin0);
        ...
        CacheBlockOut blockout1 =
            DiTCacheAgent::getinstance().on_after_block(blockin1);
    ...
    CacheStepOut stepout = DiTCacheAgent::getinstance().on_after_step(stepin1);
