...
void dump_if_not_exists(const torch::Tensor& t, const std::string& path) {
  if (fs::exists(path)) {
    return;
  }
  auto bytes = torch::pickle_save(t.cpu());
 ...
 public:
  ...
  torch::Tensor forward(torch::Tensor input) {
    ...
    if (world_size_ > 1 && gather_output_) {
      LOG(INFO) << "gather before: " << output.sizes();
      // torch::npu::synchronize();
      output =
          xllm::parallel_state::gather(output, parallel_args_.process_group_);
      // torch::npu::synchronize();
      LOG(INFO) << "gather after: " << output.sizes();
    }
    return output;
  }
 ...
 public:
  ...
  torch::Tensor forward(torch::Tensor input) {
    if (!input_is_parallelized_) {
      LOG(INFO) << "scatter before: " << input.sizes();
      // torch::npu::synchronize();
      input =
          xllm::parallel_state::scatter(input, parallel_args_.process_group_);
      // torch::npu::synchronize();
      LOG(INFO) << "scatter after: " << input.sizes();
    }
    ...
    if (if_reduce_results_ && world_size_ > 1) {
      LOG(INFO) << "reduce before: " << output.sizes();

      dump_if_not_exists(output,
                         "/export/home/zhangjun.937/code/lym/output" +
      ...
      LOG(INFO) << "reduce after: " << output.sizes();
    }
    return output;
  }
...
class DiTQKVParallelLinearImpl : public torch::nn::Module {
 public:
  ...
  void load_state_dict(const StateDict& state_dict,
    ...
    // std::vector<std::string> prefixes = {"q_proj.", "k_proj.", "v_proj."};
    if (!qkv_weight_is_loaded_) {
      LOG(INFO) << "size: " << state_dict.size() << " prefix； "
                << state_dict.prefix();
      bool all_loaded = true;
      for (size_t i = 0; i < prefixes.size(); ++i) {
        all_loaded =
        ...
        if (!all_loaded) {
          LOG(INFO) << "load model " << prefixes[i] << "size: "
                    << state_dict.get_dict_with_prefix(prefixes[i]).size()
                    << " prefix； "
                    << state_dict.get_dict_with_prefix(prefixes[i]).prefix();
        }
      }
      if (all_loaded) {
        const auto merged_weight = torch::cat(qkv_weight_list_, /*dim=*/0);
