...
class QwenImageEditPlusPipelineImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor prepare_regione_condition_latents(
      const torch::Tensor& image_latents,
      const std::vector<std::vector<int64_t>>& main_shape) const {
    if (!image_latents.defined() || main_shape.empty() ||
        main_shape[0].size() < 3) {
      return torch::Tensor();
    }

    const int64_t target_grid_h = main_shape[0][1];
    const int64_t target_grid_w = main_shape[0][2];
    const int64_t target_seq_len = target_grid_h * target_grid_w;
    if (target_grid_h <= 0 || target_grid_w <= 0 || target_seq_len <= 0 ||
        image_latents.dim() != 3) {
      return torch::Tensor();
    }

    const int64_t condition_count = static_cast<int64_t>(main_shape.size()) - 1;
    int64_t reference_index =
        ::xllm::DiTConfig::get_instance().dit_regione_reference_image_index();
    if (reference_index < 0) {
      reference_index += condition_count;
    }
    CHECK_GE(reference_index, 0)
        << "RegionE reference image index is out of range for "
        << condition_count << " condition images";
    CHECK_LT(reference_index, condition_count)
        << "RegionE reference image index is out of range for "
        << condition_count << " condition images";

    int64_t offset = 0;
    torch::Tensor reference_condition;
    for (size_t index = 1; index < main_shape.size(); ++index) {
      const auto& condition_shape = main_shape[index];
      if (condition_shape.size() < 3) {
        return torch::Tensor();
      }
      const int64_t condition_grid_h = condition_shape[1];
      const int64_t condition_grid_w = condition_shape[2];
      const int64_t condition_seq_len = condition_grid_h * condition_grid_w;
      if (condition_grid_h <= 0 || condition_grid_w <= 0 ||
          condition_seq_len <= 0 ||
          offset + condition_seq_len > image_latents.size(1)) {
        return torch::Tensor();
      }

      const int64_t condition_index = static_cast<int64_t>(index) - 1;
      if (condition_index == reference_index) {
        auto condition = image_latents.slice(
            /*dim=*/1, offset, offset + condition_seq_len);
        condition = condition
                        .reshape({image_latents.size(0),
                                  condition_grid_h,
                                  condition_grid_w,
                                  image_latents.size(2)})
                        .permute({0, 3, 1, 2});
        if (condition_grid_h != target_grid_h ||
            condition_grid_w != target_grid_w) {
          condition = torch::nn::functional::interpolate(
              condition,
              torch::nn::functional::InterpolateFuncOptions()
                  .size(std::vector<int64_t>{target_grid_h, target_grid_w})
                  .mode(torch::kBilinear)
                  .align_corners(false));
        }
        reference_condition = condition.permute({0, 2, 3, 1})
                                  .reshape({image_latents.size(0),
                                            target_seq_len,
                                            image_latents.size(2)});
      }
      offset += condition_seq_len;
    }

    if (offset != image_latents.size(1) || !reference_condition.defined()) {
      return torch::Tensor();
    }
  ...
  DiTForwardOutput forward(const DiTForwardInput& input) {
    ...
    if (regione) {
      ...
      auto regione_condition_latents =
          prepare_regione_condition_latents(image_latents, main_shape);
      regione->regione_prepare_inference(final_latents,
                                         regione_condition_latents,
                                         main_shape[0][1],
                                         main_shape[0][2],
                                         sp_rank,
                                         sp_size);
    ...
    for (int64_t i = 0; i < timesteps.size(0); ++i) {
      ...
      if (regione) {
        const auto previous_timestep =
            i > 0 ? timesteps[i - 1] : torch::Tensor();
        regione_plan = regione->begin_step(i, t, previous_timestep);
