...
class Timeline:
    ...
    def _emit_marker(self, stats: dict, pid: int) -> None:
    ...
    def _emit_memory(self, stats: dict, pid: int) -> None:
        ...
        """
        name = "memory_alloc" if stats["memoryKind"] == 1 else "memory_free"
    ...
    def _emit_kernel(self, stats: dict, pid: int) -> None:
    ...
    def _allocate_pids(self) -> None:
        ...
        for dev_stats in self._step_stats:
            ...
            if dev_stats["AscendKind"] == "MARKER":
                ...
                logger.warning(f"Unsupported AscendKind {dev_stats['AscendKind']}")
...
def parse_args():
    ...
    parser.add_argument(
        "--input", "-i", type=str, default="./node_0.log", help="Path to input log file (default: ./log/node_0.log)"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="mspti_chrome_trace.json",
